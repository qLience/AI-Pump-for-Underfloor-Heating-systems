...
class AiInputProvider:
    ...
    def calculate_ai_input(self, env_values, action):
        ...
        if self.env_decider == TL12 or self.env_decider == SETL2 or self.env_decider == ETL2:
            # Compress to state vector
            state =  [T1_std, orientation1_std, diff1_std, Tmix_std]
            
        elif self.env_decider == TL3 or self.env_decider == SETL3 or self.env_decider == ETL3:
        ...
        elif self.env_decider == TL4 or self.env_decider == SETL4 or self.env_decider == ETL4:
            # Circuit valves open or close ?
            if action ==  4:
                self.C1_valve = 1
                self.C2_valve = 1
